...
<html lang="{{app('l')}}" >
    <head>
        ...
        <title>{{ trans('client.login_page') }}</title>
            ...
            <div class="card card-outline card-primary">
                <div class="card-header text-center">
                    ...
                    <div class="alert alert-success alert-dismissible">
                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                        <h6><i class="icon fas fa-check"></i> {{ trans('Client.success') }}</h6>
                        {{ session('success') }}
                    </div>
                    @endif
                </div>
                <div class="card-body">
                    <p class="login-box-msg">{{ trans('Client.login_page') }}</p>
                    <form method="post">
                        @honeypot
                        {!! csrf_field() !!}
                        <div class="input-group mb-3" dir="{{app('dir')}}">
                            <input type="email" name="email" class="form-control" placeholder="{{ trans('Client.email') }}">
                        ...
                        <div class="input-group mb-3" dir="{{app('dir')}}">
                            <input type="password" class="form-control" name="password" placeholder="{{ trans('Client.password') }}">
                            <div class="input-group-append">
                                <div class="input-group-text">
                    ...
                    <a style="color:{{ app('l') == $lang?'#c33':'#343a40' }}" href="{{ aurl('lang/'.$lang) }}">{{ trans('Client.'.$lang) }}</a>
                    @php
                    $i++;
                    @endphp
                    {{ $i < count(L::all()) ?'.':'' }}
